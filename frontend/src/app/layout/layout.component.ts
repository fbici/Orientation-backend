...
@Component({
              ...
              @for (item of sec.items; track item.route) {
                @if (!item.roles || hasAnyRole(item.roles)) {
                  <a [routerLink]="item.route" routerLinkActive="active" [routerLinkActiveOptions]="{ exact: item.route === '/dashboard' }" class="nav-link" [attr.data-tip]="collapsed() ? item.label : null">
                    ...
                    @if (item.badge && !collapsed()) { <span class="nav-badge">{{ item.badge }}</span> }
                  </a>
                }
              }
            </div>
      ...
      <div class="main">
        <header class="topbar">
          ...
          <div class="topbar-right">
            ...
            <button class="btn btn-ghost btn-icon topbar-icon-btn" data-tip="Notifications"><span class="material-symbols-rounded">notifications</span>@if (unread()>0) { <span class="icon-badge">{{ unread() }}</span> }</button>
            @if (auth.hasRole('ADMIN')||auth.hasRole('SUPER_ADMIN')) { <a routerLink="/settings" class="btn btn-ghost btn-icon topbar-icon-btn"><span class="material-symbols-rounded">settings</span></a> }
            <div class="topbar-sep"></div>
            <div class="user-trigger" (click)="menuOpen.set(!menuOpen())">
              <div class="avatar" style="background:var(--brand);font-size:.6875rem">{{ initials() }}</div>
              <div class="user-meta"><span class="user-meta-name">{{ auth.user()?.firstName }} {{ auth.user()?.lastName }}</span><span class="user-meta-role">{{ auth.user()?.roles?.[0] }}</span></div>
            ...
            @if (menuOpen()) {
              ...
              <div class="dropdown-menu" style="min-width:220px">
                <a routerLink="/profile" class="dropdown-item" (click)="menuOpen.set(false)"><span class="material-symbols-rounded">person</span>Mon profil</a>
                <a routerLink="/settings" class="dropdown-item" (click)="menuOpen.set(false)"><span class="material-symbols-rounded">settings</span>Paramètres</a>
                <div class="dropdown-divider"></div>
                <button class="dropdown-item danger" (click)="logout()"><span class="material-symbols-rounded">logout</span>Déconnexion</button>
              </div>
            }
          </div>
        </header>
  ...
  styles: [`
    ...
    @media(max-width:1024px){.search-input{width:200px}.search-input:focus{width:260px}.user-meta{display:none}}
...
export class LayoutComponent {
  ...
  hasAnyRole(roles: string[]): boolean { return roles.some(r => this.auth.hasRole(r)); }
